...
does not silently drive the study dummies and clustering. A column already mapped
as `estimate`, `se`, or `n` is never picked: the single remaining column is used
instead, and when several remain an error asks for `study_id`. With study fixed
effects (`studylevel` 1 or 3), the data needs at least the number of unique
studies plus three rows.
