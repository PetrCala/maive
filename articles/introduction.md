...
removing completely empty rows. With study fixed effects (`studylevel` 1
or 3), there must be at least as many rows as unique studies plus three
(the fixture above has 12 rows for 4 studies).
...
as the study identifier and a warning names it. A column already mapped
as `estimate`, `se`, or `n` is never picked: when column four is one of
them, the single remaining column is used, and when several remain an
error asks for `study_id` (at `studylevel = 0` the fallback is skipped,
since no identifier is needed). If column four is a moderator or a year
rather than a study identifier, drop it or name the correct column
explicitly, because the identifier drives the study dummies and
clustering at every `studylevel` other than 0. When `studylevel` is not
0 and there is no identifier at all, a warning says the study-level
...
variance. The `ni` column and attribute are only used when they agree
with `n1i + n2i` wherever both are present, because the attribute does
not follow a reorder such as
[`dplyr::arrange()`](https://dplyr.tidyverse.org/reference/arrange.html).
Rows of an `rma.uni` fit pass through the fit’s `subset` and
...
fits are refused: their imputed studies would enter MAIVE as observed
estimates, so pass the original `rma()` fit.
